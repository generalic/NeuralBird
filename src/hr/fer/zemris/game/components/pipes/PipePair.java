...
public class PipePair extends Group implements IComponent, Comparable<PipePair> {
    ...
    public boolean intersects(Bird bird) {
        ...
        return birdBounds.intersects(upperBodyBounds) || birdBounds.intersects(lowerBodyBounds)
                || birdBounds.intersects(upperHeadBounds) || birdBounds.intersects(lowerHeadBounds);
    }
    ...
    public void randomizeYPositions() {
        ...
        double yUpper =
                random.nextDouble() * (height - gap - MINIMUM_Y_OFFSET - MINIMUM_Y_OFFSET) + MINIMUM_Y_OFFSET - height;
